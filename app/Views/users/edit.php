...
<nav class="breadcrumb-nav mb-4" aria-label="Breadcrumb">
    ...
    <span class="current">Edit: @<?= esc($user['username']) ?></span>
</nav>
<?= $this->endSection() ?>
...
<div class="max-w-2xl mx-auto">
    ...
    <!-- Card Formulir Edit Pengguna -->
    <div class="card rounded-2xl border border-slate-200/80 bg-white shadow-xs overflow-hidden">
        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl <?= ($user['role'] === 'admin') ? 'bg-indigo-50 text-indigo-600' : 'bg-emerald-50 text-emerald-600' ?>">
                ...
                <div>
                    <h2 class="text-base font-bold text-slate-900">Perbarui Profil & Peran Pengguna</h2>
                    <p class="text-xs text-slate-500 mt-0.5">Memperbarui akun: <strong class="text-slate-700"><?= esc($user['nama_lengkap']) ?></strong> (@<?= esc($user['username']) ?>)</p>
                </div>
            </div>

            <?php if ($user['id'] === (int) session()->get('user_id')): ?>
                <span class="inline-flex items-center px-2 py-1 rounded text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200/70">
                    Akun Anda Saat Ini
                </span>
            <?php endif; ?>
        </div>

        <form action="<?= base_url('users/update/' . $user['id']) ?>" method="POST" class="p-6 space-y-5">
            <?= csrf_field() ?>
            ...
            <!-- Reset Password Opsional dengan Toggle Lihat/Sembunyi -->
            <div class="rounded-xl border border-slate-200/80 bg-slate-50/50 p-4">
                <label for="password" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">
                    Ganti Password Baru <span class="text-slate-400 font-normal lowercase">(kosongkan jika tidak diubah)</span>
                </label>
                <div class="relative">
                    <input 
                        type="password" 
            ...
            <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <label class="relative flex cursor-pointer rounded-xl border border-slate-200 p-4 shadow-2xs hover:border-slate-300 has-checked:border-emerald-600 has-checked:bg-emerald-50/30 transition-all">
                        <input type="radio" name="role" value="staff" class="sr-only" <?= (old('role', $user['role']) === 'staff') ? 'checked' : '' ?>>
