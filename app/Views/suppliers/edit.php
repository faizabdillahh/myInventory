...
<div class="card form-card mx-auto max-w-3xl rounded-2xl border border-slate-200/80 bg-white shadow-xs overflow-hidden">
    <div class="card-header p-6 border-b border-slate-100">
        <h2 class="card-title text-lg font-bold text-slate-900 flex items-center gap-2.5">
            ...
            <span>Perbarui Informasi Pemasok</span>
        </h2>
        <p class="card-subtitle text-xs text-slate-500 mt-1">Perbarui data kontak, narahubung PIC, maupun alamat supplier mitra.</p>
    </div>
    ...
    <form action="<?= base_url('suppliers/update/' . $supplier['id']) ?>" method="POST" autocomplete="off" class="p-6 space-y-5">
        <?= csrf_field() ?>
        ...
        <div class="form-row grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            ...
            <div class="form-group">
                ...
                <input 
                    ...
                    value="<?= esc(old('kode_supplier', $supplier['kode_supplier'])) ?>" 
                    required
                >
            </div>
            ...
            <div class="form-group">
                ...
                <input 
                    ...
                    placeholder="Contoh: PT Mega Graha Distribusi" 
                    value="<?= esc(old('nama_supplier', $supplier['nama_supplier'])) ?>" 
                    required
                >
            </div>
        </div>
        ...
        <div class="form-row grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
            ...
            <div class="form-group">
                ...
                <input 
                    ...
                    placeholder="Contoh: Bambang Sudirgo" 
                    value="<?= esc(old('kontak_person', $supplier['kontak_person'] ?? '')) ?>"
                >
            </div>
            ...
            <div class="form-group">
                ...
                <input 
                    ...
                    name="telepon" 
                    class="form-control w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition-all placeholder-slate-400" 
                    placeholder="Contoh: 021-5558901" 
                    value="<?= esc(old('telepon', $supplier['telepon'] ?? '')) ?>"
                >
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <input 
                ...
                name="email" 
                class="form-control w-full px-3.5 py-2.5 text-xs rounded-xl border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition-all placeholder-slate-400" 
                placeholder="Contoh: sales@megagraha.co.id" 
        ...
        <div class="form-group">
            ...
            ><?= esc(old('alamat', $supplier['alamat'] ?? '')) ?></textarea>
        </div>
