...
<div class="card form-card mx-auto max-w-2xl rounded-2xl border border-slate-200/80 bg-white shadow-xs overflow-hidden">
    <div class="card-header p-6 border-b border-slate-100">
        <h2 class="card-title text-lg font-bold text-slate-900 flex items-center gap-2.5">
            ...
            <span>Perbarui Informasi Kategori</span>
        </h2>
        <p class="card-subtitle text-xs text-slate-500 mt-1">Perubahan nama kategori akan disinkronkan otomatis ke seluruh produk terkait.</p>
    </div>
    ...
    <form action="<?= base_url('categories/update/' . $category['id']) ?>" method="POST" autocomplete="off" class="p-6 space-y-5">
        <?= csrf_field() ?>
        ...
        <div class="form-group">
            ...
            <input 
                ...
                value="<?= esc(old('nama_kategori', $category['nama_kategori'])) ?>" 
                required 
                autofocus
            >
            <small class="block text-[11px] text-slate-400 mt-1">Nama kelompok kategori produk.</small>
        </div>
        ...
        <div class="form-group">
            ...
            ><?= esc(old('deskripsi', $category['deskripsi'] ?? '')) ?></textarea>
        </div>
